...
The bridge does not execute tool calls. It preserves valid function definitions, ordered tool calls and results, text, image URLs and data URIs, reasoning summaries, and required Responses controls while Codex remains responsible for execution. Function schemas that xAI would reject for the entire request are omitted from the Grok projection only; Codex catalog, tool_search history, and the Native GPT path keep the original tools. Integer-valued JSON numbers in function and tool_search arguments, such as `8.0`, are rewritten to JSON integers; real fractions are unchanged. Replayable message, function, and tool-search history is forwarded to Grok; completed Native `custom_tool_call` items and foreign reasoning are omitted. When Codex records assistant commentary inside a completed parallel tool batch, the xAI projection moves that commentary before the batch while preserving its content, call order, result order, and every `call_id`; the saved Codex history is not rewritten. On GPT/Grok switches it excludes provider-unreplayable hosted-search execution items, item IDs, and reasoning state while preserving the `call_id` links between tool calls and outputs. Grok transport establishment or early body-stream failures are retried up to three times. Native Responses and compact requests share a maximum of three retries across connection failures, timeouts, disconnects before response headers, early body-stream failures, and HTTP 429/502/503/504 responses. Backoff is 1s, 2s, and 4s. One 60-second deadline bounds sending and waiting for the first successful response-body bytes, including time spent inside those waits; no new attempt starts after the deadline. A valid upstream Retry-After delay is honored when it fits that bound. After output has started, the bridge never replays the request or cuts a healthy Native stream short because of the initial-response deadline. If Grok closes after useful Codex-consumed events without a terminal marker, the bridge synthesizes only `response.completed`; it does not invent output items, and it does not synthesize after `response.failed` or `response.incomplete`.
